...
- [Continuous-Time Deterministic Dynamic Programming](#continuous-time-deterministic-dynamic-programming)
...
The equation above is called the ==dynamic programming equation==.
  ...
  $$where $x(t)$ and $u(t)$ are simply written as $x$ and $u$.
    - ==Proof==
      ...
      $$by Taylor expansion.
      > Note that when $\hat{s} \to s$, $\frac{\omicron(x(\hat{s})-x(s))}{\hat{s}-s}=\frac{\omicron(x(\hat{s})-x(s))}{x(\hat{s})-x(s)}\frac{x(\hat{s})-x(s)}{\hat{s}-s}=\frac{\omicron(x(\hat{s})-x(s))}{x(\hat{s})-x(s)}b(s,\ x(s),\ u(s))=0$. 
...
- **Symmetric Random Walk**
  ...
  - Let $X_j=\begin{cases}
    1,& \omega_j = H \\
    -1,& \omega_j = T \\
  \end{cases}$ for outcome "Head" or "Tail" and define 
  $$
  M_k = \sum\limits_{j=1}^{k} X_j \quad (k=1,\ 2,\ \cdots)
  $$with $M_0=0$, then the process $M$ is a symmetric random walk.
  - A random walk has ==independent increments==. Each increment $M_{k_{i+1}} - M_{k_i}$ has ==expected value $0$ and variance $k_{i+1}-k_i$== since $M_{k_{i+1}} - M_{k_i} = X_{k_i+1} + X_{k_i+2} + \cdots + X_{k_{i+1}}$.
  - The symmetric random walk is a ==martingale== since $\mathrm{E}(M_{k_{i+1}}|\mathcal{F}_{k_i})=M_{k_i}$.
  - The ==quadratic variation== $[M,\ M]_k = \sum\limits_{j=1}^{k} (M_j-M_{j-1})^{2} = \sum\limits_{j=1}^{k} X_j^{2} = k$.
<br>
...
- **Scaled Symmetric Random Walk**
  ...
  - By the CLT (Central Limit Theorem), when $n\to \infty$, $M_{nt} \sim N(0,\ t)$ since $\mathrm{E}(M_{nt}) = 0$ and $\mathrm{Var}(M_{nt})=nt$. Thus, ==$W^{n}(t)\sim N(0,\ t)$ as $n\to \infty$==.
<br>
...
- **Arithmetic and Geometric Brownian Motion**
  ...
  - GBM is a limit of a Binomial model.
    - ==Proof==
      ...
      \end{aligned}
      $$
      > Note that $\sinh x = \frac{e^{x} - e^{-x}}{2}$, $\cosh x = \frac{e^{x} + e^{-x}}{2}$ and $\sinh (x-y) = \sinh x \cosh y - \cosh x \sinh y$.
      ...
      \end{aligned}
      $$
  - In a ==risk-neutral== world, the $\mu$ is just the risk-free rate per unit time, $r$.
<br>
...
- **Probability Space**
  - $\sigma$-algebra
    ...
    > Note that a $\sigma$-algebra is a field (since a field is a special ring, it is also a ring), so we also called it a $\sigma$-field. Borel field $\mathcal{B}$ is a common $\sigma$-field.
  ...
  - $\mathcal{F}$-measurable
    Given a probability space $(\Omega,\ \mathcal{F},\ \mathrm{P})$, a function $f:\ \Omega \to \mathbb{R}^{n}$ is $\mathcal{F}$-measurable if $\forall $ open sets $U \in \mathbb{R}^{n}$, $f ^{-1} (U) := \{\omega \in \Omega; f(\omega)\in U\} \in \mathcal{F}$. A ==random variable== $X$ is an $\mathcal{F}$-measurable function $X:\ \Omega \to \mathbb{R}^{n}$.
  - Distribution
