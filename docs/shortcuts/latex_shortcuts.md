...
在设置中搜索 `LaTeX Workshop > Formatting: Latex`，可选的 formatter 有 `latexindent` 和 `tex-fmt`。据说前者可以有更多灵活的设置，但后者更快更轻量。以后者 `tex-fmt` 为例，需要到 [GitHub Release](https://github.com/WGUNDERWOOD/tex-fmt/releases) 下载对应的可执行文件，并将其路径（所在文件夹）添加到环境变量中。之后把 VSCode 关掉重开，用 `Alt + Shift + F` 就可以格式化了。

> [!NOTE|label:注意]
> 不能只用 `Developer: Reload Window` 来重载窗口，亲测无效。
...
某些字母用 `\bm` 加粗后，如果下标不加粗，下标与字母之间的距离会过大，可以使用 `\!` 为下标添加负距离。目前已知的字母有：$\bm{C},\ \bm{D},\ \bm{f},\ \bm{I},\ \bm{V},\ \bm{Y}$。
