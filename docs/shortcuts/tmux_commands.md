...
- `tmux set mouse on`：支持鼠标功能，包括：
    - 滚轮滑动：进入滚动模式，可以用滚轮或方向键操控，按 `Esc` 退出；
    - 点击选中 panel：直接点击 panel 即可选中，无需使用快捷键切换；
    - 右键菜单：右键按住 panel 会弹出菜单，拖到对应选项上松手即可执行对应操作。

    > [!NOTE|注意]
    > 这不代表鼠标可以像正常 terminal 那样使用，复制文字依旧需要按住 `Shift` 来选中。
