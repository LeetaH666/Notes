...
如果 $\bm{C}_{\! \bm{f}}$ 可逆，由 $\eqref{3}$ 式可得 $\bm{\lambda}_{\bm{f}} = \bm{C}_{\! \bm{f}}^{-1}  \bm{\mu}_{\bm{R}}$。由于 $\bm{C}_{\! \bm{f}}$ 是实对称矩阵，通过写成 $\bm{\lambda}_{\bm{f}} = (\bm{C}_{\! \bm{f}}^{\top} \bm{C}_{\! \bm{f}})^{-1} \bm{C}_{\! \bm{f}} \bm{\mu}_{\bm{R}}$，我们发现**因子的风险价格可以通过截面回归得到**：
...
> [!TIP|label:提示]
> 在抽样得到 $\bm{\mu}_{\bm{Y}}$ 后，作者做了标准化的操作，即将 $\bm{\mu}_{\bm{Y}}$ 中每一个元素都除以对应的标准差，对于 test assets $\bm{R}$ 来说，这一操作相当于把它们的均值变成了夏普比，这与 [Shrinking the cross-section](papers\shrinking_the_cross-section.md#sdf-系数的压缩估计量) 中对夏普比做岭回归有异曲同工之妙——都是对变量做标准化。
